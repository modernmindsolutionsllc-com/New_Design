      ...
      <header className={`navbar ${isScrolled ? 'navbar--scrolled' : ''}`}>
        <div className="container navbar__inner">
          <Link to="/" className="navbar__logo" aria-label={`${COMPANY_NAME} home`}>
            <img src={logoSrc} alt={COMPANY_NAME} className="navbar__logo-image" />
          </Link>
...
.navbar {
  ...
  background: linear-gradient(180deg, color-mix(in srgb, var(--color-bg) 94%, white 6%) 0%, var(--color-bg) 100%);
...
.navbar__logo {
  ...
  background: color-mix(in srgb, var(--color-bg-white) 88%, transparent 12%);
...
.navbar__link:hover {
  ...
  background: color-mix(in srgb, var(--color-gold-subtle) 75%, transparent 25%);
}
...
.theme-toggle {
  ...
  border-radius: 50%;
  border: 1px solid var(--color-border);
  background: linear-gradient(145deg, var(--color-bg-white), var(--color-bg-subtle));
  color: var(--color-text-primary);
  display: inline-flex;
  align-items: center;
  justify-content: center;
...
.mobile-menu {
  ...
  background:
    radial-gradient(circle at 85% 12%, color-mix(in srgb, var(--color-gold) 14%, transparent 86%) 0%, transparent 36%),
    var(--color-bg-white);
  z-index: 200;
