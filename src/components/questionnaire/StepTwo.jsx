 ...
 * Fields: service type cards, project description, starting from, features checklist
 */
  ...
  >
    <motion.div className="form-step__header" variants={FADE_UP}>
      ...
      <p className="form-step__sub">
        No technical knowledge needed — plain words are perfect.
      </p>
    </motion.div>

    <motion.div className="form-step__fields" variants={FADE_UP}>

      {/* Service type — visual checkbox cards */}
          ...
          {SERVICE_TYPES.map(({ id, label, emoji }) => {
            ...
            return (
              <button
                ...
                onClick={() => { toggleArrayField('serviceTypes', id); clearFieldError('serviceTypes') }}
                aria-pressed={selected}
              >
                <span className="service-type-card__emoji" aria-hidden>{emoji}</span>
                <span className="service-type-card__label">{label}</span>
                {selected && <span className="service-type-card__check" aria-hidden>✓</span>}
              </button>
            )
          })}
        </div>
      ...
      <div className={`form-field ${fieldHasError(errors, 'projectDescription') ? 'form-field--error' : ''}`}>
        ...
        <textarea
          ...
          rows={5}
          placeholder="e.g. I run a small bakery and I want an app where customers can pre-order cakes and pay online. I don't have an existing website yet."
      ...
      {/* Starting from scratch or existing */}
      <div className={`form-field ${fieldHasError(errors, 'startingFrom') ? 'form-field--error' : ''}`}>
        <label className="form-field__label">
          Are you starting from scratch or have an existing product?
          <span className="form-field__required">*</span>
        </label>
        <div className="radio-group">
          {[
            { value: 'scratch',  label: '🆕 Starting from scratch', sub: 'No existing code or design' },
            { value: 'existing', label: '🔧 Already have something', sub: 'Existing website, app, or codebase' },
          ].map(({ value, label, sub }) => (
            <label
              key={value}
              className={`radio-card ${formData.startingFrom === value ? 'radio-card--selected' : ''}`}
            >
              <input
                type="radio" name="startingFrom" value={value}
                checked={formData.startingFrom === value}
                onChange={() => { updateField('startingFrom', value); clearFieldError('startingFrom') }}
      ...
      <div className="form-field">
        <label className="form-field__label">
          ...
          <span className="form-field__hint"> — tick anything that sounds relevant</span>
        </label>
        <div className="checkbox-grid">
          {FEATURE_OPTIONS.map(feature => {
            ...
            return (
              <label
                key={feature}
                className={`checkbox-item ${checked ? 'checkbox-item--checked' : ''}`}
              >
                <input
                  type="checkbox"
                  checked={checked}
...
.service-type-card__emoji { font-size: 1.5rem; }
.service-type-card__label {
  font-size: var(--text-xs); font-weight: 600;
  color: var(--color-text-secondary); line-height: 1.3;
}
.service-type-card--selected .service-type-card__label { color: var(--color-gold-dark); }
.service-type-card__check {
  position: absolute; top: 6px; right: 8px;
  font-size: 0.65rem; font-weight: 700;
  color: var(--color-gold); background: var(--color-bg-white);
...
.radio-card:hover  { border-color: var(--color-gold-border); background: var(--color-gold-subtle); }
.radio-card--selected { border-color: var(--color-gold); background: var(--color-gold-subtle); }
.radio-card__input { accent-color: var(--color-gold); width: 18px; height: 18px; flex-shrink: 0; }
.radio-card__label { font-size: var(--text-sm); font-weight: 600; color: var(--color-text-primary); font-family: var(--font-body); }
.radio-card__sub   { font-size: var(--text-xs); color: var(--color-text-muted); margin-top: 2px; font-family: var(--font-body); }
...
.checkbox-item:hover      { border-color: var(--color-gold-border); }
.checkbox-item--checked   { border-color: var(--color-gold); background: var(--color-gold-subtle); }
.checkbox-item__input     { display: none; }
.checkbox-item__box {
  width: 20px; height: 20px; border-radius: var(--radius-sm);
  border: 2px solid var(--color-border); background: var(--color-bg-white);
  display: flex; align-items: center; justify-content: center;
  font-size: 0.65rem; font-weight: 900; color: var(--color-gold);
...
.checkbox-item--checked .checkbox-item__label { color: var(--color-gold-dark); font-weight: 600; }
