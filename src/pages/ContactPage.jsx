...
import { CONTACT_EMAIL, CONTACT_PHONE, COMPANY_ADDRESS, WHATSAPP_URL, STAGGER_CONTAINER, FADE_UP } from '@utils/constants'
...
const CONTACT_METHODS = [
  ...
  { icon: MapPin,       label: 'Based In',    value: COMPANY_ADDRESS,   href: null },
...
const ContactPage = () => {
  ...
  const validate = () => {
    const e = {}
    const n = validateName(fields.name);   if (!n.valid) e.name = n.message
    const em = validateEmail(fields.email); if (!em.valid) e.email = em.message
    const m = validateRequired(fields.message, 'Message'); if (!m.valid) e.message = m.message
    return e
  }
  ...
  const handleSubmit = async (ev) => {
    ...
    await submitToFormspree({
      ...fields,
      _subject: `Contact form: ${fields.subject || 'General enquiry'} from ${fields.name}`,
      ...
      <section className="page-hero section--gray">
        <div className="container">
          <motion.div className="page-hero__inner" variants={STAGGER_CONTAINER} initial="hidden" animate="visible">
            ...
            <motion.h1 className="page-hero__heading" variants={FADE_UP}>Let's Talk</motion.h1>
            <div className="gold-divider" />
            <motion.p className="page-hero__sub" variants={FADE_UP}>
              Whether you have a project in mind or just a question — we're real people
              and we actually reply. Usually within a few hours.
            </motion.p>
          </motion.div>
        </div>
      </section>
                  ...
                  <div>
                    ...
                    {href ? (
                      <a href={href} target={href.startsWith('http') ? '_blank' : undefined}
                        rel="noopener noreferrer" className="contact-method-card-v2__value contact-method-card-v2__value--link">
                        {value}
                      </a>
                    ) : (
                ...
                <form className="contact-form" onSubmit={handleSubmit} noValidate>
                  ...
                  <div className="contact-form__row">
                    <div className={`form-field ${errors.name ? 'form-field--error' : ''}`}>
                      ...
                      <input id="c-name" type="text" className="form-field__input"
                        placeholder="Priya Sharma" value={fields.name}
                    ...
                    <div className={`form-field ${errors.email ? 'form-field--error' : ''}`}>
                      ...
                      <input id="c-email" type="email" className="form-field__input"
                        placeholder="you@company.com" value={fields.email}
                  ...
                  <div className="form-field">
                    ...
                    <input id="c-subject" type="text" className="form-field__input"
                      placeholder="e.g. Question about pricing"
                      value={fields.subject} onChange={e => update('subject', e.target.value)} />
                  </div>
                  ...
                  <div className={`form-field ${errors.message ? 'form-field--error' : ''}`}>
                    ...
                    <textarea id="c-message" className="form-field__input form-field__textarea"
...
.contact-page__methods { display: flex; flex-direction: column; gap: var(--space-4); }
...
.contact-method-card-v2:hover { border-color: var(--color-gold-border); box-shadow: var(--shadow-sm); }
...
.contact-method-card-v2__value--link {
  color: var(--color-gold-dark); text-decoration: none; transition: var(--transition-base);
}
.contact-method-card-v2__value--link:hover { color: var(--color-gold); }
...
.contact-page__form-wrap {
  background: var(--color-bg-white); border: 1px solid var(--color-border);
  border-radius: var(--radius-xl); padding: var(--space-10); box-shadow: var(--shadow-md);
}
...
.contact-form__submit:hover { background: var(--color-gold-dark); border-color: var(--color-gold-dark); box-shadow: var(--shadow-gold); }
.contact-form__submit:disabled { opacity: 0.7; cursor: not-allowed; }

.contact-page__success {
  display: flex; flex-direction: column; align-items: center;
  gap: var(--space-4); text-align: center; padding: var(--space-16);
}
.contact-page__success-icon {
  width: 72px; height: 72px; border-radius: 50%;
  background: var(--color-gold-subtle); border: 2px solid var(--color-gold-border);
  color: var(--color-gold); font-size: 2rem; font-weight: 700;
